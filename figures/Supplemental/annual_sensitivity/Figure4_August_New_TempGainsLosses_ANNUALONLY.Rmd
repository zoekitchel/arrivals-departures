...
(gain_preds_temp_only <- ggplot() +
  geom_ribbon(data = temp_gains_newdata, aes(x = seas_sst_temp_scaled, ymin = exp(plo)/(1+exp(plo)), ymax = exp(phi)/(1+exp(phi))), fill = "lightgrey") +
  geom_point(data = spp_master_ztemp_seus_buoy_scaled_annual_obs, aes(x = seas_sst_temp_scaled, y = col_03), size = 2, shape = "|") +
  geom_line(data = temp_gains_newdata, aes(x = seas_sst_temp_scaled, y=exp(col)/(1+exp(col)))) +
