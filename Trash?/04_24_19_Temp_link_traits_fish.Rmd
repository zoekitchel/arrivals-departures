...
ggplot(data=spp_master_ztemp, aes(x=year, y=mean_sst_temp, col = reg)) +
  geom_point() +
  labs(x="Year", y="Mean Annual Surface\n Temperature (˚C)") +
  ylim(0,25) +
...
```
Get to know facet wrap but with facet wrap instead
```{r temp over time}
#temperature through time
ggplot(data=spp_master_ztemp, aes(x=year)) +
  geom_line(aes(y = min_sst_temp, col = "blue")) +
  geom_line(aes(y = max_sst_temp, col = "red")) +
  geom_line(aes(y = seas_sst_temp, col = "grey")) +
...
mod_null <- glmer(col ~ (1|reg) + (1|year_factor), family = binomial, data = spp_master_ztemp)
...
#get rid of mean
colonization_model_comparisons <- colonization_model_comparisons %>%
  filter(!grepl("mean", variable))

colonization_model_comparisons <- data.table(colonization_model_comparisons)

#add ∆AIC to tables
min_col_AICc <- min(colonization_model_comparisons[, AICc])
colonization_model_comparisons[,"deltaAICc" := (AICc - min_col_AICc)]

#add relative likelihood exp( -0.5 * ∆AIC score for that model)
colonization_model_comparisons[,"rel_likelihood" := exp((-0.5 * deltaAICc))]
#sum relative likelihoods across all models
colonization_model_comparisons.likelihoodsum <- sum(colonization_model_comparisons[, rel_likelihood])
#Akaike weight for a model is this rel_likelihood devided by the sum of these values across all models
colonization_model_comparisons[,"akaike_weight" := rel_likelihood/colonization_model_comparisons.likelihoodsum]

#I want to look at relative importance FOR 
#bottom/surface (bottom = includes sbt, grepl("sbt", variable))
col_bottom_importance <- sum(colonization_model_comparisons[grep("sbt", variable), ]$akaike_weight)
col_surface_importance <- sum(colonization_model_comparisons[grep("sst", variable), ]$akaike_weight)

#lag/not lagged (grepl("lag", variable))
col_lag_importance <- sum(colonization_model_comparisons[grep("lag", variable), ]$akaike_weight)

#absolute (grepl("abs", variable))
col_abs_importance <- sum(colonization_model_comparisons[grep("abs", variable), ]$akaike_weight)

#raw (!grepl("change"))
col_raw_importance <- sum(colonization_model_comparisons[!grep("change", variable), ]$akaike_weight)

#change (grepl("change", variable), (!grepl("abs")))
col_change_importance <- (1 - (col_abs_importance + col_raw_importance))
...
#get rid of mean
extinction_model_comparisons <- extinction_model_comparisons %>%
  filter(!grepl("mean", variable))

extinction_model_comparisons <- data.table(extinction_model_comparisons)

#add ∆AIC to tables
min_ext_AICc <- min(extinction_model_comparisons[, AICc])
extinction_model_comparisons[,"deltaAICc" := (AICc - min_ext_AICc)]

#add relative likelihood exp( -0.5 * ∆AIC score for that model)
extinction_model_comparisons[,"rel_likelihood" := exp((-0.5 * deltaAICc))]
#sum relative likelihoods across all models
extinction_model_comparisons.likelihoodsum <- sum(extinction_model_comparisons[, rel_likelihood])
#Akaike weight for a model is this rel_likelihood devided by the sum of these values across all models
extinction_model_comparisons[,"akaike_weight" := rel_likelihood/extinction_model_comparisons.likelihoodsum]

#I want to look at relative importance FOR 
#bottom/surface (bottom = includes sbt, grepl("sbt", variable))
ext_bottom_importance <- sum(extinction_model_comparisons[grep("sbt", variable), ]$akaike_weight)
ext_surface_importance <- sum(extinction_model_comparisons[grep("sst", variable), ]$akaike_weight)

#lag/not lagged (grepl("lag", variable))
ext_lag_importance <- sum(extinction_model_comparisons[grep("lag", variable), ]$akaike_weight)

#absolute (grepl("abs", variable))
ext_abs_importance <- sum(extinction_model_comparisons[grep("abs", variable), ]$akaike_weight)

#raw (!grepl("change"))
ext_raw_importance <- sum(extinction_model_comparisons[!grep("change", variable), ]$akaike_weight)
...
#colonization
lags_v_RVI[1,2] <- 1-sum(colonization_model_comparisons[grep("lag", variable), ]$akaike_weight)
lags_v_RVI[2,2] <- sum(colonization_model_comparisons[grep("lag1", variable), ]$akaike_weight)
lags_v_RVI[3,2] <- sum(colonization_model_comparisons[grep("lag2", variable), ]$akaike_weight)
lags_v_RVI[4,2] <- sum(colonization_model_comparisons[grep("lag3", variable), ]$akaike_weight)
lags_v_RVI[5,2] <- sum(colonization_model_comparisons[grep("lag4", variable), ]$akaike_weight)
lags_v_RVI[6,2] <- sum(colonization_model_comparisons[grep("lag5", variable), ]$akaike_weight)
lags_v_RVI[7,2] <- sum(colonization_model_comparisons[grep("lag6", variable), ]$akaike_weight)
lags_v_RVI[8,2] <- sum(colonization_model_comparisons[grep("lag7", variable), ]$akaike_weight)
lags_v_RVI[9,2] <- sum(colonization_model_comparisons[grep("lag8", variable), ]$akaike_weight)
lags_v_RVI[10,2] <- sum(colonization_model_comparisons[grep("lag9", variable), ]$akaike_weight)
#extinction
lags_v_RVI[1,3] <- 1-sum(extinction_model_comparisons[grep("lag", variable), ]$akaike_weight)
lags_v_RVI[2,3] <- sum(extinction_model_comparisons[grep("lag1", variable), ]$akaike_weight)
lags_v_RVI[3,3] <- sum(extinction_model_comparisons[grep("lag2", variable), ]$akaike_weight)
lags_v_RVI[4,3] <- sum(extinction_model_comparisons[grep("lag3", variable), ]$akaike_weight)
lags_v_RVI[5,3] <- sum(extinction_model_comparisons[grep("lag4", variable), ]$akaike_weight)
lags_v_RVI[6,3] <- sum(extinction_model_comparisons[grep("lag5", variable), ]$akaike_weight)
lags_v_RVI[7,3] <- sum(extinction_model_comparisons[grep("lag6", variable), ]$akaike_weight)
lags_v_RVI[8,3] <- sum(extinction_model_comparisons[grep("lag7", variable), ]$akaike_weight)
lags_v_RVI[9,3] <- sum(extinction_model_comparisons[grep("lag8", variable), ]$akaike_weight)
lags_v_RVI[10,3] <- sum(extinction_model_comparisons[grep("lag9", variable), ]$akaike_weight)

ggplot(data = lags_v_RVI, aes(x = col_lag)) +
