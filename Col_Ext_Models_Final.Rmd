...
load("spp_master_ztemp_seus_buoy_seus_buoys.RData")
```
...
colnames(extinction_model_comparison_spprandom_scaled_seus_update_nomean)
#add null to tables
...
colonization_model_comparison_spprandom_scaled_seus_update_nomean <- rbind(colonization_model_comparison_spprandom_scaled_seus_update_nomean, col_null, use.names = F)

extinction_model_comparison_spprandom_scaled_seus_update_nomean <- rbind(extinction_model_comparison_spprandom_scaled_seus_update_nomean, ext_null, use.names = F)
```
...
type <- c("Depth", "Depth", "lag", "lag", "Transformation", "Transformation", "Transformation", "Measurement", "Measurement", "Measurement")
...
RVI_table[, variable := factor(variable, levels = c("Bottom", "Surface", "|Change|", "Change", "Raw", "no_lag", "lagged", "Min", "Max", "Seas"))]
...
#colonization
RVI_col_plot <- ggplot(data = RVI_table[col_ext == "col" & type != "lag",], aes(x=type, y = value, fill = variable)) +
  geom_bar(stat="identity", color = "black", size = 0) +
  scale_x_discrete(breaks = c("Transformation", "Depth", "Measurement"), labels = c("Transformation", "Depth", "Measurement")) +
  labs(x = "Temperature Variables", y = "Relative Variable Importance") +
  theme_classic() +
  theme(text = element_text(size = 20)
...
#extinction
RVI_ext_plot <- ggplot(data = RVI_table[col_ext == "ext" & type != "lag",], aes(x=type, y = value, fill = variable)) +
  geom_bar(stat="identity", color = "black", size = 0) +
  scale_x_discrete(breaks = c("Transformation", "Depth", "Measurement"), labels = c("Transformation", "Depth", "Measurement")) +
  labs(x = "Temperature Variables", y = "Relative Variable Importance") +
  theme_classic()  +
...
bestcolmod_scaled <- glmer(col ~ max_sbt_temp_change_abs_scaled + (1|reg) + (1|spp), family = binomial, data = spp_master_ztemp_seus_buoy_scaled, nAGQ = 0)
...
model_imputs <- spp_master_ztemp_seus_buoy_scaled[,c("spp", "reg", "year", "col", "now_ext", "max_sbt_temp_change_abs_scaled")]
...
bestextmod_scaled <- glmer(now_ext ~ min_sst_temp_change_lag2_scaled + (1|reg) + (1|spp), family = binomial, data = spp_master_ztemp_seus_buoy_scaled, nAGQ = 0)
...
#plotting predicted versus modeled
#unique combinations of spp, region, and max_sbt_temp_change_abs_scaled found in dataset
model_imputs <- spp_master_ztemp_seus_buoy_scaled[,c("spp", "reg", "year", "col", "now_ext", "min_sst_temp_change_lag2_scaled")]
