...
#from colonizations
cumulative_richness <- col_model_comparisons[,.(reg, year, col_obs_sum, col_pred_sum)]
...
cumulative_richness[, Observed := col_obs_sum-ext_obs_sum][, Predicted := col_pred_sum-ext_pred_sum]

cumulative_richness.r <- cumulative_richness[,.(reg, year, Observed, Predicted)]

#wide to long
cumulative_richness.r <- melt(cumulative_richness.r, id.vars = c("reg", "year"), measure.vars = c("Observed", "Predicted"), variable.name = "Richness", value.name = "value")
...
cum_richness_plot <- ggplot(aes(x=year, y = value, color = Richness), data = cumulative_richness.r) +
  geom_point(size = 1) +
  geom_hline(yintercept = 0) +
  labs(x="Year", y = "Cumulative Richness") +
  facet_wrap(~reg) +
  theme_classic() +
  scale_color_manual(values = c("Observed" = 'coral3' , "Predicted" = 'deepskyblue4')) +
