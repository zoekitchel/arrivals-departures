...
Here, I scaled all temperature values WITHIN A REGION, and included species as a random effect. In order to do this, I had to change the algorithm slightly--nAGQ = 0.
...
spp_master_ztemp_varsonly[, paste0(tempvars, "_scaled") := lapply(.SD, function(x) as.vector(scale(x))), by = reg]
...
#min_sbt_temp_lag7_scaled
#max_sbt_temp_change_abs_scaled
#seas_sst_temp_scaled

bestcolmod <- glmer(col ~ min_sbt_temp_lag7_scaled + (1|reg) + (1|spp), family = binomial, data = spp_master_ztemp_scaled, nAGQ = 0)
...
newdd_col$merge <- paste(newdd$spp, newdd$reg, sep = "_")
temp.values_col <- seq(min(spp_master_ztemp_scaled$min_sbt_temp_lag7_scaled), max(spp_master_ztemp_scaled$min_sbt_temp_lag7_scaled), by = 0.05)
...
names(newdd.full_col) <- c("spp", "reg", "min_sbt_temp_lag7_scaled")

t2_col <- predict(bestcolmod, newdata = newdd.full_col, type = "response", re.form = NA)

newdd.full_col$pred <- t2_col


ggplot(aes(x=min_sbt_temp_lag7_scaled, y = pred), data = newdd.full_col) +
  geom_point()

#plot actual observations
#round to nearest 0.05
spp_master_ztemp_scaled[,"min_sbt_temp_lag7_scaled_round" := round_any(min_sbt_temp_lag7_scaled, 0.05, round)]

#now, i can get # of colonizations per reg/spp/temp combo
#number of observations per unique reg/spp/temp groups
observations <- spp_master_ztemp_scaled[, .N, by = .(spp, reg, min_sbt_temp_lag7_scaled_round)] #number of observations per spp, reg, 0.05 temp 

observations_all <- spp_master_ztemp_scaled[, .N, by = .(min_sbt_temp_lag7_scaled_round)]
observations_all[,"obs_count" := N]

colonizations <- spp_master_ztemp_scaled[, sum(col), by = .(spp, reg, min_sbt_temp_lag7_scaled_round)] #number of colonizations per spp, reg, 0.05 temp 

colonizations_all <- spp_master_ztemp_scaled[, sum(col), by = .(min_sbt_temp_lag7_scaled_round)]
...
colonization_observations$min_sbt_temp_lag7_scaled_round2 <- colonization_observations$min_sbt_temp_lag7_scaled_round
colonization_observations <- colonization_observations[,7:8]

ggplot(aes(x=min_sbt_temp_lag7_scaled, y = pred), data = newdd.full_col) +
  geom_point() +
  geom_point(data = colonization_observations, aes(x = min_sbt_temp_lag7_scaled_round2, y = proportion), col = "red") +
  theme_classic()
...
newdd.full_ext <- expand.grid(newdd$merge, temp.values)
...
t2_ext <- predict(bestextmod, newdata = newdd.full_ext, type = "response", re.form = NA)
...
t2_col <- predict(bestcolmod_notscaled, newdata = newdd.full_col, type = "response", re.form = NA)
