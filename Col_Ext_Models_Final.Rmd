...
colnames(extinction_model_comparison_spprandom_scaled_nomean)
#add null to tables
interceptcolmod_scaled <- glmer(col ~ (1|spp) + (1|reg), family = binomial, nAGQ = 0, data = spp_master_ztemp_scaled)
interceptextmod_scaled <- glmer(now_ext ~ (1|reg) + (1|spp), family = binomial, data = spp_master_ztemp_scaled, nAGQ = 0)


col_null <- data.frame("null", "-", "-", AICc(interceptcolmod_scaled))
ext_null <- data.frame("null", "-", "-", AICc(interceptextmod_scaled))

colonization_model_comparison_spprandom_scaled_nomean <- rbind(colonization_model_comparison_spprandom_scaled_nomean, col_null, use.names = F)

extinction_model_comparison_spprandom_scaled_nomean <- rbind(extinction_model_comparison_spprandom_scaled_nomean, ext_null, use.names = F)
```
