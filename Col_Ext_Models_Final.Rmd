...
spp_master_ztemp <- data.table(spp_master_ztemp)

#merge spp master and yes no fish
spp_master_ztemp_fishonly <- spp_master_ztemp[fish_nofishkey, on = c("reg", "spp")]
spp_master_ztemp_fishonly2 <- spp_master_ztemp_fishonly[fish == "Y"]

dim(spp_master_ztemp_fishonly2)

#all species
inverts <- setdiff(unique(spp_master_ztemp$spp), unique(spp_master_ztemp_fishonly2$spp))
```
...
merge_keys <- speciesnames_withedits[fish_nofishkey, on = c("inverts" = "spp")]
merge_keys$invert <- na.replace(merge_keys$invert, no)

merge_keys[,fish_update := ifelse(fish == "Y" & invert == "no" | na.omit(invert), "Y", "N")]
