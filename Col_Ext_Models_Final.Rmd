...
We had some in the original fish/no fish key that were indeed fish but marked as inverts

```{r import updated list of inverts}
speciesnames_withedits <- data.table(read.csv("invert_yes_no_manualedits.csv"))
colnames(speciesnames_withedits) <- c("spp", "invert")

load("fish_nofishkey.RData")
fish_nofishkey <- data.table(fish_nofishkey[,2:4])

merge_keys <- speciesnames_withedits[fish_nofishkey, on = c("spp")]


merge_keys[invert == "no", fish := "Y"] #if it ended up in inverts list, but not an invert, make a fish



```

##Run models again, but with only fish, no inverts

```{r getting rid of non fish}

#merge spp master and yes no fish
spp_master_ztemp_fishonly <- spp_master_ztemp[merge_keys, on = c("reg", "spp")]
spp_master_ztemp_fishonly2 <- spp_master_ztemp_fishonly[fish == "Y"]
...
#names of temp variables, so I can add "_scaled" suffix
tempvars <- colnames(spp_master_ztemp_fishonly2[,45:292])

#extract just temperature variables to scale
spp_master_ztemp_varsonly <- spp_master_ztemp_fishonly2[,c(1,45:292)]
...
#merge new columns back with spp_master_ztemp
spp_master_ztemp_fishonly_scaled <- cbind(spp_master_ztemp_fishonly2, spp_master_ztemp_varsonly[,scaled_cols, with = FALSE])

#going to make loop to make all models I need to look at with single variables
variables_scaled <- colnames(spp_master_ztemp_fishonly_scaled[,scaled_cols, with = FALSE])
...
for (i in 1:length(variables_scaled)){
  mod <- glmer(col ~ get(variables_scaled[i]) + (1|reg) + (1|spp), family = binomial, data = spp_master_ztemp_fishonly_scaled, nAGQ = 0)
...
col_mod_null <- glmer(col ~ (1|reg) + (1|spp), family = binomial, data = spp_master_ztemp_fishonly_scaled, nAGQ = 0)
AICc(col_mod_null)
min(colonization_model_comparison_spprandom_scaled_fishonly$AICc)
```
Extinction Models without Traits
...
for (i in 1:length(variables_scaled)){
  mod <- glmer(now_ext ~ get(variables_scaled[i]) + (1|reg) + (1|spp), family = binomial, data = spp_master_ztemp_fishonly_scaled, nAGQ = 0)
...
ext_mod_null <- glmer(ext ~ (1|reg) + (1|spp), family = binomial, data = spp_master_ztemp_fishonly_scaled, nAGQ = 0)
AICc(ext_mod_null)
min(extinction_model_comparison_spprandom_scaled_fishonly$AICc)
```
Relative Variable Importance
```{r get rid of any models using mean for fish only}
colonization_model_comparison_spprandom_scaled_nomean <- colonization_model_comparison_spprandom_scaled_fishonly[!grepl("mean",variable),]

extinction_model_comparison_spprandom_scaled_nomean <- extinction_model_comparison_spprandom_scaled_fishonly[!grepl("mean",variable),]
```

Relative Variable Importance for Colonization Models
```{r RVI colonization for fish only}
#add ∆AIC to tables
min_col_AICc <- min(colonization_model_comparison_spprandom_scaled_nomean_fishonly[, AICc])
colonization_model_comparison_spprandom_scaled_nomean_fishonly[,"deltaAICc" := (AICc - min_col_AICc)]

#add relative likelihood exp( -0.5 * ∆AIC score for that model)
colonization_model_comparison_spprandom_scaled_nomean_fishonly[,"rel_likelihood" := exp((-0.5 * deltaAICc))]
#sum relative likelihoods across all models
colonization_model_comparison_spprandom_scaled_nomean_fishonly.likelihoodsum <- sum(colonization_model_comparison_spprandom_scaled_nomean_fishonly[, rel_likelihood])
#Akaike weight for a model is this rel_likelihood devided by the sum of these values across all models
colonization_model_comparison_spprandom_scaled_nomean_fishonly[,"akaike_weight" := rel_likelihood/colonization_model_comparison_spprandom_scaled_nomean_fishonly.likelihoodsum]

#I want to look at relative importance FOR 
#bottom/surface (bottom = includes sbt, grepl("sbt", variable))
col_bottom_importance_fishonly <- sum(colonization_model_comparison_spprandom_scaled_nomean_fishonly[grep("sbt", variable), ]$akaike_weight)
col_surface_importance_fishonly <- sum(colonization_model_comparison_spprandom_scaled_nomean_fishonly[grep("sst", variable), ]$akaike_weight)

#lag/not lagged (grepl("lag", variable))
col_lag_importance_fishonly <- sum(colonization_model_comparison_spprandom_scaled_nomean_fishonly[grep("lag", variable), ]$akaike_weight)
col_nolag_importance_fishonly <- sum(colonization_model_comparison_spprandom_scaled_nomean_fishonly[!grep("lag", variable), ]$akaike_weight)

#absolute (grepl("abs", variable))
col_abs_importance_fishonly <- sum(colonization_model_comparison_spprandom_scaled_nomean_fishonly[grep("abs", variable), ]$akaike_weight)

#raw (!grepl("change"))
col_raw_importance_fishonly <- sum(colonization_model_comparison_spprandom_scaled_nomean_fishonly[!grep("change", variable), ]$akaike_weight)

#change (grepl("change", variable), (!grepl("abs")))
col_change_importance_fishonly <- sum(colonization_model_comparison_spprandom_scaled_nomean_fishonly[grep("change", variable), ][!grep("abs", variable),]$akaike_weight)

#type of temp variable (max, min, seas)
col_max_temp_importance_fishonly <- sum(colonization_model_comparison_spprandom_scaled_nomean_fishonly[grep("max", variable), ]$akaike_weight)
col_min_temp_importance_fishonly <- sum(colonization_model_comparison_spprandom_scaled_nomean_fishonly[grep("min", variable), ]$akaike_weight)
col_seas_temp_importance_fishonly <- sum(colonization_model_comparison_spprandom_scaled_nomean_fishonly[grep("seas", variable), ]$akaike_weight)
```
Relative Variable Importance for Extinction Models
```{r RVI extinction for fish only}
#add ∆AIC to tables
min_ext_AICc_fishonly <- min(extinction_model_comparison_spprandom_scaled_nomean_fishonly[, AICc])
extinction_model_comparison_spprandom_scaled_nomean_fishonly[,"deltaAICc" := (AICc - min_ext_AICc)]

#add relative likelihood exp( -0.5 * ∆AIC score for that model)
extinction_model_comparison_spprandom_scaled_nomean_fishonly[,"rel_likelihood" := exp((-0.5 * deltaAICc))]
#sum relative likelihoods across all models
extinction_model_comparison_spprandom_scaled_nomean_fishonly.likelihoodsum <- sum(extinction_model_comparison_spprandom_scaled_nomean_fishonly[, rel_likelihood])
#Akaike weight for a model is this rel_likelihood devided by the sum of these values across all models
extinction_model_comparison_spprandom_scaled_nomean_fishonly[,"akaike_weight" := rel_likelihood/extinction_model_comparison_spprandom_scaled_nomean_fishonly.likelihoodsum]

#I want to look at relative importance FOR 
#bottom/surface (bottom = includes sbt, grepl("sbt", variable))
ext_bottom_importance_fishonly <- sum(extinction_model_comparison_spprandom_scaled_nomean_fishonly[grep("sbt", variable), ]$akaike_weight)
ext_surface_importance_fishonly <- sum(extinction_model_comparison_spprandom_scaled_nomean_fishonly[grep("sst", variable), ]$akaike_weight)

#lag/not lagged (grepl("lag", variable))
ext_lag_importance_fishonly <- sum(extinction_model_comparison_spprandom_scaled_nomean_fishonly[grep("lag", variable), ]$akaike_weight)
ext_nolag_importance_fishonly <- sum(extinction_model_comparison_spprandom_scaled_nomean_fishonly[!grep("lag", variable), ]$akaike_weight)

#absolute (grepl("abs", variable))
ext_abs_importance_fishonly <- sum(extinction_model_comparison_spprandom_scaled_nomean_fishonly[grep("abs", variable), ]$akaike_weight)

#raw (!grepl("change"))
ext_raw_importance_fishonly <- sum(extinction_model_comparison_spprandom_scaled_nomean_fishonly[!grep("change", variable), ]$akaike_weight)

#change (grepl("change", variable), (!grepl("abs")))
ext_change_importance_fishonly <- sum(extinction_model_comparison_spprandom_scaled_nomean_fishonly[grep("change", variable), ][!grep("abs", variable),]$akaike_weight)

#type of temp variable (max, min, seas)
ext_max_temp_importance_fishonly <- sum(extinction_model_comparison_spprandom_scaled_nomean_fishonly[grep("max", variable), ]$akaike_weight)
ext_min_temp_importance_fishonly <- sum(extinction_model_comparison_spprandom_scaled_nomean_fishonly[grep("min", variable), ]$akaike_weight)
ext_seas_temp_importance_fishonly <- sum(extinction_model_comparison_spprandom_scaled_nomean_fishonly[grep("seas", variable), ]$akaike_weight)
```
...
#colonization
lags_v_RVI_spprandom_scaled_fishonly[1,2] <- 1-sum(colonization_model_comparison_spprandom_scaled_nomean_fishonly[grep("lag", variable), ]$akaike_weight)
lags_v_RVI_spprandom_scaled_fishonly[2,2] <- sum(colonization_model_comparison_spprandom_scaled_nomean_fishonly[grep("lag1", variable), ]$akaike_weight)
lags_v_RVI_spprandom_scaled_fishonly[3,2] <- sum(colonization_model_comparison_spprandom_scaled_nomean_fishonly[grep("lag2", variable), ]$akaike_weight)
lags_v_RVI_spprandom_scaled_fishonly[4,2] <- sum(colonization_model_comparison_spprandom_scaled_nomean_fishonly[grep("lag3", variable), ]$akaike_weight)
lags_v_RVI_spprandom_scaled_fishonly[5,2] <- sum(colonization_model_comparison_spprandom_scaled_nomean_fishonly[grep("lag4", variable), ]$akaike_weight)
lags_v_RVI_spprandom_scaled_fishonly[6,2] <- sum(colonization_model_comparison_spprandom_scaled_nomean_fishonly[grep("lag5", variable), ]$akaike_weight)
lags_v_RVI_spprandom_scaled_fishonly[7,2] <- sum(colonization_model_comparison_spprandom_scaled_nomean_fishonly[grep("lag6", variable), ]$akaike_weight)
lags_v_RVI_spprandom_scaled_fishonly[8,2] <- sum(colonization_model_comparison_spprandom_scaled_nomean_fishonly[grep("lag7", variable), ]$akaike_weight)
lags_v_RVI_spprandom_scaled_fishonly[9,2] <- sum(colonization_model_comparison_spprandom_scaled_nomean_fishonly[grep("lag8", variable), ]$akaike_weight)
lags_v_RVI_spprandom_scaled_fishonly[10,2] <- sum(colonization_model_comparison_spprandom_scaled_nomean_fishonly[grep("lag9", variable), ]$akaike_weight)
#extinction
lags_v_RVI_spprandom_scaled_fishonly[1,3] <- 1-sum(extinction_model_comparison_spprandom_scaled_nomean_fishonly[grep("lag", variable), ]$akaike_weight)
lags_v_RVI_spprandom_scaled_fishonly[2,3] <- sum(extinction_model_comparison_spprandom_scaled_nomean_fishonly[grep("lag1", variable), ]$akaike_weight)
lags_v_RVI_spprandom_scaled_fishonly[3,3] <- sum(extinction_model_comparison_spprandom_scaled_nomean_fishonly[grep("lag2", variable), ]$akaike_weight)
lags_v_RVI_spprandom_scaled_fishonly[4,3] <- sum(extinction_model_comparison_spprandom_scaled_nomean_fishonly[grep("lag3", variable), ]$akaike_weight)
lags_v_RVI_spprandom_scaled_fishonly[5,3] <- sum(extinction_model_comparison_spprandom_scaled_nomean_fishonly[grep("lag4", variable), ]$akaike_weight)
lags_v_RVI_spprandom_scaled_fishonly[6,3] <- sum(extinction_model_comparison_spprandom_scaled_nomean_fishonly[grep("lag5", variable), ]$akaike_weight)
lags_v_RVI_spprandom_scaled_fishonly[7,3] <- sum(extinction_model_comparison_spprandom_scaled_nomean_fishonly[grep("lag6", variable), ]$akaike_weight)
lags_v_RVI_spprandom_scaled_fishonly[8,3] <- sum(extinction_model_comparison_spprandom_scaled_nomean_fishonly[grep("lag7", variable), ]$akaike_weight)
lags_v_RVI_spprandom_scaled_fishonly[9,3] <- sum(extinction_model_comparison_spprandom_scaled_nomean_fishonly[grep("lag8", variable), ]$akaike_weight)
lags_v_RVI_spprandom_scaled_fishonly[10,3] <- sum(extinction_model_comparison_spprandom_scaled_nomean_fishonly[grep("lag9", variable), ]$akaike_weight)
...
bestcolmod_scaled_fishonly <- glmer(col ~ max_sst_temp_change_abs_lag1_scaled + (1|reg) + (1|spp), family = binomial, data = spp_master_ztemp_fishonly_scaled, nAGQ = 0)
...
#plotting predicted versus modeled
#unique combinations of spp, region, and max_sbt_temp_change_abs_scaled found in dataset
model_imputs <- spp_master_ztemp_fishonly_scaled[,c("spp", "reg", "year", "col", "now_ext", "max_sst_temp_change_abs_lag1_scaled")]
...
Predicted versus observed for 1|spp models for fish only scaled
...
bestextmod_scaled_fishonly <- glmer(now_ext ~ min_sbt_temp_change_abs_lag5_scaled + (1|reg) + (1|spp), family = binomial, data = spp_master_ztemp_fishonly_scaled, nAGQ = 0)
...
#plotting predicted versus modeled
#unique combinations of spp, region, and max_sbt_temp_change_abs_scaled found in dataset
model_imputs <- spp_master_ztemp_fishonly_scaled[,c("spp", "reg", "year", "col", "now_ext", "min_sbt_temp_change_abs_lag5_scaled")]
...
Once I decide whether or not to scale... I should add in traits again
