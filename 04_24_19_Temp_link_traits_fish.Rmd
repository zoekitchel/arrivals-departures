...
One idea would just be to semarate as sessile and mobile by invertebrate/fish
Best Model Colonization:
col_mod_change_abs_1_var <- glmer(col ~ sst_seas_change_abs + (1 | reg) + (1 | year), family = binomial, data = spp_master_ztemp)

```{r}
col_mod_change_abs_1_var_sessilemobile <- glmer(col ~ sst_seas_change_abs*fish + (1 | reg) + (1 | year), family = binomial, data = spp_master_ztemp_fishnofish)
