...
Now, let's play around with lags a bit using dredge, this didn't work because of NA's, I guess I'll compare one by one then??
...
First, let's make a model with changes from the year before last
```{r}
...
```
Both incorporating lags in changes are equally poor, so we'll stick with most recent changes. Next step, I will take away variables and see what's best for colonization using AIC
```{r}
