...
df_reg[, lapply(.SD,mean), by=list(year,region,month)] #monthly mean across region

#now, max, min, seas, mean across years
setDT(df_reg)[ , list(region_year_mean_sst = mean(region_month_mean_sst), region_year_max_sst = max(region_month_mean_sst)), region_year_min_sst = mean(region_month_mean_sst),region_year_seas_sst = region_year_max_sst - region_year_min_sst , by = .(region, year)]

date_temp_reg_avg <- df_reg %>%
