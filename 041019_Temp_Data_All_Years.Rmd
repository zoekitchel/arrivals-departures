...
#soda_sst <- readRDS("SODA2.1.6_SST.rds") to load instead
```

New column in newlatlongyear with "X1958.01.01" format.
```{r adjusting to date key format}
...
latlongreg.key.u <- unique(latlongreg.key[,c('lat','lon', 'region')])

```

Then I will link data table with GPS and date and temp with regions using above key, and then take appropriate averages

```{r linking data to region to take avg across regions with left join}
#left join
df_reg <- merge(df_long, latlongreg.key.u)

df_reg[, temp_month_mean:=mean(temp), by=list(year,region,month)] #monthly mean across region

#now, max, min, seas, mean across years
df_reg[ , region_year_mean_sst := mean(temp_month_mean), by = list(year, region)][ , region_year_max_sst := max(temp_month_mean), by = list(year, region)][ , region_year_min_sst := min(temp_month_mean), by = list(year, region)][ , region_year_seas_sst := region_year_max_sst - region_year_min_sst, by = list(year, region)]  #max, mean, min, seas

save(df_reg, file = "date_ssttemp_avg_SODA.Rdata") #save this so i can start here!

ggplot(data=df_reg, aes(x=year, y=region_year_mean_sst)) +
  geom_point() +
  facet_wrap(~region)
```
...
nm1 <- grep("*sst*", colnames(df_reg), value=TRUE)
...
date_temp_reg_avg_withlags <- data.table(df_reg)
...
#empty memory space
rm(dat, df, df_long, df_reg, hauls, locations, soda_sst, temp_values)
```
...
#now I can get rid of years before 1963
#first make sure years is numeric
date_temp_reg_avg_withlags$year <- as.numeric(date_temp_reg_avg_withlags$year)
fin_temps_wlags <- date_temp_reg_avg_withlags[year > 1962]
```
