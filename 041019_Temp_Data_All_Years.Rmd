...
for (i in 1:length(regions)) {
  subset <- latlongyear[latlongyear$region == regions[i],] #subset to a region, ct 2478
  ...
  subsetlatlong.noreg <- subsetlatlong[,c(1:2)] #don't need column with location or year anymore, 1488
  #randomly choose 1/5 of these
  quarter <- 0.20*nrow(subsetlatlong.noreg)
  subsetlatlong.reduced <- subsetlatlong.noreg[sample(nrow(subsetlatlong.noreg), quarter), ]
  
  allmonths <- c(1:12) #we want temp values for every month
  
  subsetlatlong.m <- subsetlatlong.reduced %>%
