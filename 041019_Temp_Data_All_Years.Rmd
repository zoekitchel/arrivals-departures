...
for (i in 1:length(regions)) {
  subset <- latlongyear[latlongyear$region == regions[i],] #subset to a region
  ...
  allyears <- seq(newminyear, maxyear) #years we need to simulate GPS points for
  ...
  subsetlatlong.noreg <- subsetlatlong[,c(1:2)] #don't need column with location or year anymore
  
  allmonths <- c(1:12) #we want temp values for every month
  
  subsetlatlong.m <- subsetlatlong.noreg %>%
    unite("latlon", c("lat", "lon") , sep = "_", remove = TRUE) #merge lat lon in order to expand grid (essentially, lat_lon becomes a key)
  ...
  tempdf <- expand.grid(subsetlatlong.m.vector, allyears, allmonths) #matches every year value with a lat_lon_month
  
  tempdf.sep <- separate(tempdf, Var1, c("lat", "lon"), sep = "_", remove = T)
...
#it's possible at some point I may need to randomly sample these, because 8736684 extractions may be too many...
...
locations <- SpatialPoints(cbind(latlongyear_10yrboostIDonly$lon, latlongyear_10yrboostIDonly$lat), 
...
df_long <- gather(df_na_rm, key = "ID", value = "temp", 3:674 )
head(df_long)
```
