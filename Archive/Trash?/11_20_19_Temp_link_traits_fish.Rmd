...
colonization_model_comparisons.nomean <- colonization_model_comparisons.nomean[!grepl("lag10", variable), ]
...
col_max_temp_importance <- sum(colonization_model_comparisons.nomean[grep("max", variable), ]$akaike_weight)
col_min_temp_importance <- sum(colonization_model_comparisons.nomean[grep("min", variable), ]$akaike_weight)
col_seas_temp_importance <- sum(colonization_model_comparisons.nomean[grep("seas", variable), ]$akaike_weight)

#save this table
save(colonization_model_comparisons.nomean, file = "colonization_models_with_likelihoods.Rdata")
```
...
#only raw temp has lag of 10 years, which means variable importance comparisons are unbalanced so I will get rid of these rows
extinction_model_comparisons.nomean <- extinction_model_comparisons.nomean[!grepl("lag10", variable), ]
...
count(extinction_model_comparisons.nomean[grep("abs", variable), ])

#raw (!grepl("change"))
ext_raw_importance <- sum(extinction_model_comparisons.nomean[!grep("change", variable), ]$akaike_weight)
count(extinction_model_comparisons.nomean[!grep("change", variable), ])
...
#type of temp variable (max, min, seas)
ext_max_temp_importance <- sum(extinction_model_comparisons.nomean[grep("max", variable), ]$akaike_weight)
count(extinction_model_comparisons.nomean[grep("max", variable), ])
ext_min_temp_importance <- sum(extinction_model_comparisons.nomean[grep("min", variable), ]$akaike_weight)
count(extinction_model_comparisons.nomean[grep("min", variable), ])
ext_seas_temp_importance <- sum(extinction_model_comparisons.nomean[grep("seas", variable), ]$akaike_weight)
count(extinction_model_comparisons.nomean[grep("seas", variable), ])

save(extinction_model_comparisons.nomean, file = "extinction_models_with_likelihoods.Rdata")
...
save(RVI_col_plot, RVI_ext_plot, RVI_lags, file = "RVI_plots.Rdata")

setEPS()
postscript("RVI_col_plot.eps")
RVI_col_plot
dev.off()

setEPS()
postscript("RVI_ext_plot.eps")
RVI_ext_plot
dev.off()

setEPS()
postscript("RVI_lags_plot.eps")
RVI_lags
dev.off()
```
...
bestcolmod_OG <- glmer(col ~ max_sbt_temp_change_abs + (1|reg) + (1|year_factor), family = binomial, data = spp_master_ztemp, nAGQ = 0)
...
model_imputs <- spp_master_ztemp[,c("spp", "reg", "year_factor", "year", "col", "now_ext", "max_sbt_temp_change_abs")]

t2_col <- predict(bestcolmod_OG, newdata = model_imputs, type = "response", re.form = NULL) #setting re.form to NULL includes all random effects
...
col_model_comparisons_OG <- model_imputs[, .(col_predicted = sum(pred), col_observed = sum(col)), by=.(year,reg)]


ggplot(aes(x=year), data = col_model_comparisons_OG) +
  geom_point(aes(y = col_observed), data = col_model_comparisons_OG, col = "red") +
  geom_point(data = col_model_comparisons_OG, aes(y = col_predicted), col = "black") +
  facet_wrap(~reg) +
  theme_classic()
...
Same, predicted versus observed, but for extinctions
```{r predicted vs. observed extinctions}
#best extinction models (all delta less than 2)
  #seas_sbt_temp_change_abs_lag9


bestextmod_OG <- glmer(now_ext ~ seas_sbt_temp_change_abs_lag9 + (1|reg) + (1|year_factor), family = binomial, data = spp_master_ztemp, nAGQ = 0)
...
#plotting predicted versus modeled
#unique combinations of spp, region, and max_sbt_temp_change_abs_scaled found in dataset
model_imputs <- spp_master_ztemp[,c("spp", "reg", "year_factor", "year", "col", "now_ext", "seas_sbt_temp_change_abs_lag9")]

t2_ext <- predict(bestextmod_OG, newdata = model_imputs, type = "response", re.form = NULL) #setting re.form to NULL includes all random effects

model_imputs$pred <- t2_ext

#group by region and year, sum over "col" = observations, and "pred" = predict colonizations by summing probabilities
ext_model_comparisons_OG <- model_imputs[, .(ext_predicted = sum(pred), ext_observed = sum(now_ext)), by=.(year,reg)]


ggplot(aes(x=year), data = ext_model_comparisons_OG) +
  geom_point(aes(y = ext_observed), data = ext_model_comparisons_OG, col = "red") +
  geom_point(data = ext_model_comparisons_OG, aes(y = ext_predicted), col = "black") +
  facet_wrap(~reg) +
  theme_classic()
