...
col <- spp_master_ztemp %>%
  group_by(reg) %>%
  summarise(n.col = sum(col))

#count extinctions by region
ext <- spp_master_ztemp %>%
  group_by(reg) %>%
  summarise(n.ext = sum(now_ext))
...
#count % colonizations by region
col_per <- spp_master_ztemp %>%
  group_by(reg) %>%
  summarise(col.p = round(sum(col)/n(), 2))

#count % extinctions by region
ext_per <- spp_master_ztemp %>%
  group_by(reg) %>%
  summarise(ext.p = round(sum(now_ext)/n(), 2))

#first year
year.first <- spp_master_ztemp %>%
  group_by(reg) %>%
  summarise(year.first = min(year))
#last year
year.last <- spp_master_ztemp %>%
  group_by(reg) %>%
  summarise(year.last = max(year))

region_summaries <- cbind(count, col[,2], col_per[,2], ext[,2], ext_per[,2], year.first[,2], year.last[,2])
```
Now we'll visualize regions in a plot
```{r region plot}
#manipulate data
region_summaries.m <- melt(region_summaries[,c(1:3,5)], id.vars = "reg")

col_ext_v_observations <- ggplot(aes(x=reg, y = value, fill = variable), data = region_summaries.m) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = "Region", y = "Observations") +
  theme_classic() +
  theme(text = element_text(size = 20))

ggsave(col_ext_v_observations, file = "col_ext_v_observations.eps")
  
region_summaries.m

#total number of colonizations
total_colonizations <- sum(region_summaries$n.col)
total_extinctions <- sum(region_summaries$n.ext)
total_observations <- sum(region_summaries$n)

total_colonizations/total_observations
total_extinctions/total_observations
total_observations
