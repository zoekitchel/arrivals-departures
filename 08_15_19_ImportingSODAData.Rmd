...
*STOPPED HERE*
First, convert grid in soda 3.3.1 from 0 360 to -180 180
```{bash convert grid}
#bottom
for i in `cat soda3.3.1_mn_ocean_reg_import.txt`; do ncks -O -v longitude --msa -d longitude,0.25,359.75 "${i%.*}_sbt.nc" "${i%.*}_sbt_rotate.nc"; done 
for i in `cat soda3.3.1_mn_ocean_reg_import.txt`; do ncap2 -O -s 'where(longitude > 180) longitude=longitude-360' "${i%.*}_sbt.nc" "${i%.*}_sbt.nc";done
ncks -C -H -v longitude ~/nco/data/in.nc
ncks --trd -C -H -v LatLon ~/out.nc
#surface
for i in `cat soda3.3.1_mn_ocean_reg_import.txt`; do ncap2 -O -s 'where(longitude > 180) longitude=longitude-360' "${i%.*}_sst.nc" "${i%.*}_sst.nc";done
```
...
#surface
ncks -O -d latitude,24.25,62.25 -d longitude,170.75,-43.75 soda3.3.1_1980_2015_sst.nc soda3.3.1_1980_2015_sst.nc

#bottom
ncks -O -d latitude,24.25,62.25 -d longitude,170.75,-43.75 soda3.3.1_1980_2015_sst.nc soda3.3.1_1980_2015_sbt.nc
```
...
************
Merging files (surface - surface, bottom - bottom)
```{bash merge two surface and two bottom temperature files}
