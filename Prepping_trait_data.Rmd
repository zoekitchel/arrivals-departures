...
#restrict to LME 1:9 (not 4, Gulf of CA) and 65 according to https://www.st.nmfs.noaa.gov/ecosystems/lme/
traits_NA <- traits[LME %in% c(65,1,5,2,7,9,6,8,3)]

#remove duplicates
traits.u <- unique(traits_NA[,.(LME, taxon,tl, age.maturity, length.max, age.max)])

```
Interested in looking at max age, trophic level, and age of maturity
```{r trait patterns}
ggplot(aes(x=age.max), data = traits.u) +
  geom_histogram()

ggplot(aes(x=age.maturity), data = traits.u) +
  geom_histogram()

ggplot(aes(x=tl), data = traits.u) +
  geom_histogram()

cor(na.omit(traits.u[,.(age.max, age.maturity, length.max, tl)]))
``` 
...
```{r linking species}
load(here::here("Data","Spp_master","spp_master_ztemp_seus_buoys.Rdata")) #load in observation data for arrivals and departures

#assigning regions to LMEs
reg <- levels(spp_master_ztemp_seus_buoy$reg)
LME <- c(65,1,5,2,7,9,6,8,3)

reg_LME_key <- data.table(reg = as.factor(reg), LME = LME)

spp_master_ztemp_seus_buoy <- spp_master_ztemp_seus_buoy[reg_LME_key, on = "reg"]
...
spp_master_ztemp_seus_buoy.traits <- spp_master_ztemp_seus_buoy[traits.u, nomatch = 0, on = c("spp","LME")]
```
