...
* setting nACQ to 0, see why (As  of August 26, 2022, this is no longer necessary because I'm using glmmTMB instead of GLMER) [here](https://stats.stackexchange.com/questions/77313/why-cant-i-match-glmmTMB-family-binomial-output-with-manual-implementation-of-g). 
...
##Here, I scaled all temperature values across all regions, and included species as a random effect. In order to do this, I had to change the algorithm slightly--nAGQ = 0.(No longer true as of 8/26/22 because using glmmTMB instead of glmer)
...
for (i in 1:length(variables_scaled)){
  mod <- glmmTMB(col ~ get(variables_scaled[i]) + (1|reg) + (1|phylum) + (1|class) + (1|order) + (1|family) + (1|genus) + (1|spp),
               family = binomial, data = spp_master_ztemp_seus_buoy_scaled)
  arrival_model_comparison_taxonomy[i,variable := variables_scaled[i]]
  arrival_model_comparison_taxonomy[i,coef := coef(summary(mod))$cond[,"Estimate"][[2]]]
  arrival_model_comparison_taxonomy[i,p_value := coef(summary(mod))$cond[,"Pr(>|z|)"][[2]]]
...
arrival_model_comparison_taxonomy <- readRDS(here("Model_results","arrival_model_results_taxonomy.rds"))

#let's take a look at best performing model, and check assumptions
mod_extrarandom <- glmmTMB(col ~ seas_sst_temp_scaled  + (1|reg) + (1|phylum) + (1|class) + (1|order) + (1|family) + (1|genus) + (1|spp), family = binomial, data = spp_master_ztemp_seus_buoy_scaled)

#R-Squared
r.squaredGLMM(mod_extrarandom)
...
#delta (observational) variance:
  #the variance explained by the fixed effects: 0.02
  #a variance explained by the entire model, including both fixed and random effects: 0.19
...
for (i in 1:length(variables_scaled)){
  mod <- glmmTMB(now_ext ~ get(variables_scaled[i]) + (1|reg) + (1|phylum) + (1|class) + (1|order) + (1|family) + (1|genus) + (1|spp), family = binomial, data = spp_master_ztemp_seus_buoy_scaled)
  departure_model_comparison_taxonomy[i,variable := variables_scaled[i]]
  departure_model_comparison_taxonomy[i,coef := coef(summary(mod))$cond[,"Estimate"][[2]]]
  departure_model_comparison_taxonomy[i,p_value := coef(summary(mod))$cond[,"Pr(>|z|)"][[2]]]
...
departure_model_comparison_taxonomy <- readRDS(here("Model_results","departure_model_results_taxonomy.rds"))

#let's take a look at best performing model, and check assumptions
mod_extrarandom_loss <- glmmTMB(now_ext ~ seas_sst_temp_lag3_scaled  + (1|reg) + (1|phylum) + (1|class) + (1|order) + (1|family) + (1|genus) + (1|spp), family = binomial, data = spp_master_ztemp_seus_buoy_scaled)

#R-Squared
r.squaredGLMM(mod_extrarandom_loss)

#theoretical variance:
  #the variance explained by the fixed effects: 0.017800760
  #a variance explained by the entire model, including both fixed and random effects: 0.4442535

#delta (observational) variance:
  #the variance explained by the fixed effects: 0.004274866
  #a variance explained by the entire model, including both fixed and random effects: 0.1066878
...
```{r null models}
#add null to tables
intercept_arrival_mod_scaled <- glmmTMB(col ~ + (1|reg) + (1|phylum) + (1|class) + (1|order) + (1|family) + (1|genus) + (1|spp), family = binomial, data = spp_master_ztemp_seus_buoy_scaled)

intercept_departure_mod_scaled <- glmmTMB(now_ext ~ + (1|reg) + (1|phylum) + (1|class) + (1|order) + (1|family) + (1|genus) + (1|spp), family = binomial, data = spp_master_ztemp_seus_buoy_scaled)

#best model better than null? yes
AICc(intercept_arrival_mod_scaled, mod_extrarandom)
AICc(intercept_departure_mod_scaled, mod_extrarandom_loss)
...
for (i in 1:length(variables_scaled)){
  mod <- glmmTMB(col ~ get(variables_scaled[i]) + (1|reg) + (1|spp), family = binomial, data = spp_master_ztemp_seus_buoy_scaled_fishonly2)
...
for (i in 1:length(variables_scaled)){
  mod <- glmmTMB(now_ext ~ get(variables_scaled[i]) + (1|reg) + (1|spp), family = binomial, data = spp_master_ztemp_seus_buoy_scaled_fishonly2)
...
#comparison to null
col_mod_null <- glmmTMB(col ~ (1|reg) + (1|spp), family = binomial, data = spp_master_ztemp_seus_buoy_scaled_fishonly2)
AICc(col_mod_null)-min(arrival_model_comparison_spprandom_scaled_seus_update_fishonly_nomean$AICc)

ext_mod_null <- glmmTMB(ext ~ (1|reg) + (1|spp), family = binomial, data = spp_master_ztemp_seus_buoy_scaled_fishonly2)
