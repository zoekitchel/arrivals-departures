...
fwrite(arrival_model_comparison, file = here("Model_results","arrival_model_results.csv"))
...
mod_sppregrandom <- glmer(col ~ seas_sst_temp_scaled  + (1|reg) + (1|spp), family = binomial, data = spp_master_ztemp_seus_buoy_scaled, nAGQ = 0)


#R-Squared
r.squaredGLMM(mod_sppregrandom)

#theoretical variance:
  #the variance explained by the fixed effects: 0.0395
  #a variance explained by the entire model, including both fixed and random effects: 0.43

#delta (observational) variance:
  #the variance explained by the fixed effects: 0.01149
  #a variance explained by the entire model, including both fixed and random effects: 0.1255

```

Gain/Arrival Models with Additional random effects

NB: August 13, 2022: These models seem to be running, but much slower than models with just a random effect for region and species. 
...
arrival_model_comparison_taxonomy <- readRDS(here("Model_results","arrival_model_results.rds"))

#let's take a look at best performing model, and check assumptions
mod_extrarandom <- glmer(col ~ seas_sst_temp_scaled  + (1|reg) + (1|phylum) + (1|class) + (1|order) + (1|family) + (1|genus) + (1|spp), family = binomial, data = spp_master_ztemp_seus_buoy_scaled, nAGQ = 0)

#R-Squared
r.squaredGLMM(mod_extrarandom)

#theoretical variance:
  #the variance explained by the fixed effects: 0.0407
  #a variance explained by the entire model, including both fixed and random effects: 0.44

#delta (observational) variance:
  #the variance explained by the fixed effects: 0.0176
  #a variance explained by the entire model, including both fixed and random effects: 0.1935
...
fwrite(departure_model_comparison, file = here("Model_results","departure_model_results.csv"))
...
for (i in 205:length(variables_scaled)){
...
AICc(intercept_arrival_mod_scaled)-min(arrival_model_comparison_nomean_akaikeweights$AICc)
AICc(intercept_departure_mod_scaled)-min(departure_model_comparison_nomean_akaikeweights$AICc)
