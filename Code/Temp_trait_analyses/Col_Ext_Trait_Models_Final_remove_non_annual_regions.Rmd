---
title: "Arrival and Departure Final Models with Traits Included: EXCLUDING AI, GOA, WCTRI"
...
ggplot(spp_master_ztemp_seus_buoy.traits_scaled.unique[!(reg %in% c("ai","goa","wctri"))][!(reg %in% c("ai","goa","wctri"))], aes(x = age.maturity))  +
  geom_histogram() +
  theme_classic()

summary(spp_master_ztemp_seus_buoy.traits_scaled.unique[!(reg %in% c("ai","goa","wctri"))][!(reg %in% c("ai","goa","wctri"))]$age.maturity)
```
...
arrival.reduced_annual_obs.r <- arrival.reduced_annual_obs[complete.cases(arrival.reduced_annual_obs[ , .(age.max_scaled, age.maturity_scaled, length.max_scaled, tl_scaled)]),]
...
all.temp.variables <- colnames(arrival.reduced_annual_obs.r[,9:256])
...
saveRDS(arrival_model_comparison_traits, here::here("Model_results","Traits","arrival_model_comparison_traits_annual_obs.rds"))
...
#delete any observations without full trait data
departure.reduced_annual_obs.r <- departure.reduced_annual_obs[complete.cases(departure.reduced_annual_obs[ , .(age.max_scaled, age.maturity_scaled, length.max_scaled, tl_scaled)]),]
...
saveRDS(departure_model_comparison_traits, here::here("Model_results","Traits","departure_model_comparison_traits_annual_obs.rds"))
...
#save this table
saveRDS(arrival_model_comparisons_traits.lag9max, here:here("Model_results","Traits","arrival_models_traits_with_likelihoods_annual_obs.rds"))
saveRDS(arrival_model_comparisons_traits.lag9max, here:here("tables","arrival_models_traits_with_likelihoods_annual_obs.rds"))
```
...
#save this table
saveRDS(departure_model_comparisons_traits.lag9max, here:here("Model_results","Traits","departure_models_traits_with_likelihoods_annual_obs.rds"))
saveRDS(departure_model_comparisons_traits.lag9max, here:here("tables","departure_models_traits_with_likelihoods_annual_obs.rds"))
```
...
mod_arrival_trait <- glmer(col ~ max_sst_temp_change_abs_lag1_scaled * age.max_scaled + (1|reg) + (1|spp), family = "binomial", data = arrival.reduced_annual_obs.r, nAGQ = 0)
...
mod_arrival_trait_notemp <- glmer(col ~ age.max_scaled + (1|reg) + (1|spp), family = "binomial", data = arrival.reduced_annual_obs.r, nAGQ = 0)
summary(mod_arrival_trait_notemp)

#reverse to age maturity unscaled
mod_arrival_reverse_scaled_agematurity <- lm(age.maturity ~ age.maturity_scaled, data = spp_master_ztemp_seus_buoy.traits_scaled.6regions) 

mod_arrival_forward_scaled_agematurity <- lm(age.maturity_scaled ~ age.maturity, data = spp_master_ztemp_seus_buoy.traits_scaled.6regions)

#reverse to get age max unscaled
mod_arrival_reverse_scaled_agemax <- lm(age.max ~ age.max_scaled, data = spp_master_ztemp_seus_buoy.traits_scaled.6regions)

mod_arrival_forward_scaled_agemax <- lm(age.max_scaled ~ age.max, data = spp_master_ztemp_seus_buoy.traits_scaled.6regions)
...
temp_agemax_newdata_mean <- mean(spp_master_ztemp_seus_buoy.traits_scaled.6regions$age.max_scaled, na.rm = T)

temp_agematurity_newdata_mean  <- mean(spp_master_ztemp_seus_buoy.traits_scaled.6regions$age.maturity_scaled, na.rm = T)
...
ggsave(arrival_preds, path = here::here("figures/Figure5"), file = "predicted_probs_arrival_maxage_annual_obs.jpg")
ggsave(arrival_preds, path = here::here("figures/Figure5"), file = "predicted_probs_arrival_maxage_annual_obs.eps")


```

##departure with Traits
Best model: max_sst_temp_scaled * age.maturity_scaled

Best model from analyses with temperature only: seas_sst_temp_lag3_scaled

```{r predicted values departure}
#best model with fish only 
mod_departure_trait <- glmer(now_ext ~ max_sst_temp_scaled * age.maturity_scaled + (1|reg) + (1|spp), family = "binomial", data = departure.reduced_annual_obs.r, nAGQ = 0)

#using best predictor for all species in model instead
mod_departure_trait_allspp <- glmer(now_ext ~ seas_sst_temp_lag3_scaled * age.maturity_scaled + (1|reg) + (1|spp), family = "binomial", data = departure.reduced_annual_obs.r, nAGQ = 0)

#reverse to agematurity unscaled
mod_departure_reverse_scaled_agematurity <- lm(age.maturity ~ age.maturity_scaled, data = spp_master_ztemp_seus_buoy.traits_scaled.6regions) 

mod_departure_forward_scaled_agematurity <- lm(age.maturity_scaled ~ age.maturity, data = spp_master_ztemp_seus_buoy.traits_scaled.6regions)

age_match <- data.table(ages_we_want = c(0.5, 1, 3, 15,25))
age_match[,age.maturity_scaled := ages_we_want * mod_departure_forward_scaled_agematurity$coefficients[2]+mod_departure_forward_scaled_agematurity$coefficients[1]]

#best predictor for fish only: max_sst_temp_change_abs_lag1_scaled as x axis
temp_departure_newdata <- runif(500,min(spp_master_ztemp_seus_buoy.traits_scaled.6regions$max_sst_temp_scaled), max(spp_master_ztemp_seus_buoy.traits_scaled.6regions$max_sst_temp_scaled))

temp_agematurity_newdata <- age_match[,age.maturity_scaled]


temp_agemax_newdata <- mean(spp_master_ztemp_seus_buoy.traits_scaled.6regions$age.max_scaled, na.rm = T)

departure_newdata_traits <- expand.grid("max_sst_temp_scaled" = temp_departure_newdata, "age.maturity_scaled" = temp_agematurity_newdata, "age.max_scaled" = temp_agemax_newdata)
...
#I want x axis to be meaningful
mod_departure_reverse_scaledtemp <- lm(max_sst_temp ~ max_sst_temp_scaled, data = spp_master_ztemp_seus_buoy.traits_scaled.6regions)
...
ggsave(path = here::here("figures/Figure5"), file = "predicted_probs_departure_maxage_annual_obs.jpg")
ggsave(path = here::here("figures/Figure5"), file = "predicted_probs_departure_maxage_annual_obs.eps")

#___________
#best predictor for all organisms: seasonality as x axis, lagged 3 years
temp_departure_newdata_seas <- runif(500,min(spp_master_ztemp_seus_buoy.traits_scaled.6regions$seas_sst_temp_lag3_scaled), max(spp_master_ztemp_seus_buoy.traits_scaled.6regions$seas_sst_temp_lag3_scaled))

temp_agematurity_newdata_seas <- age_match[,age.maturity_scaled]


temp_agemax_newdata_seas <- mean(spp_master_ztemp_seus_buoy.traits_scaled.6regions$age.max_scaled, na.rm = T)
...
#I want x axis to be meaningful
mod_departure_reverse_scaledtemp_seas <- lm(seas_sst_temp_lag3 ~ seas_sst_temp_lag3_scaled, data = spp_master_ztemp_seus_buoy.traits_scaled.6regions)
...
ggsave(width = 6, height = 2.5, path = here::here("figures/Figure5"), file = "Figure5_margeffect_annual_obs.jpg")
ggsave(width = 6, height = 2.5, path = here::here("figures/Figure5"), file = "Figure5_margeffect_annual_obs.eps")
