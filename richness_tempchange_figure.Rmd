...
spp_master_ztemp[,richness:=sum(present), by = .(year, reg)]

#raw
spp_master_ztemp[,bt_trend_max:=trend(max_sbt_temp,year), by=reg] #average yearly bottom temp
spp_master_ztemp[,bt_trend_min:=trend(min_sbt_temp,year), by=reg] #average yearly bottom temp
spp_master_ztemp[,bt_trend_seas:=trend(seas_sbt_temp,year), by=reg] #average yearly bottom temp
spp_master_ztemp[,bt_trend_mean:=trend(mean_sbt_temp,year), by=reg] #average yearly bottom temp
spp_master_ztemp[,st_trend_mean:=trend(mean_sst_temp,year), by=reg] #average yearly bottom temp
...
```{r summary data}
# create summary data
spp_master_richness <- unique(spp_master_ztemp[,.(reg, year, richness,bt_trend_max, bt_trend_min, bt_trend_mean, bt_trend_seas, st_trend_mean)])
...
Zoe's temp
```{r zoe temp plots}
ggplot(data = spp_master_ztemp, aes(x = year, y = mean_sbt_temp)) +
  geom_point() +
  facet_wrap(~reg) +
  theme_bw()
