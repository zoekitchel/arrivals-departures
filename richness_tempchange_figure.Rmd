...
```{r summary data}
# create summary data
spp_master_richness <- unique(spp_master_ztemp[,.(reg, year, richness,bt_trend_max, bt_trend_min, bt_trend_mean, bt_trend_seas)])
...
trends_zoe <- spp_master_richness[,.(rich=trend(richness, year), #calculate trend in richness per year and trend in annual temp per year
                                 ...
                                 bt_trend_seas_raw = mean(bt_trend_seas)),  by=reg]
...
ggplot() +
  ...
  geom_point(data = trends, aes(x = abs(temp), y = rich_naive, col = reg)) +
