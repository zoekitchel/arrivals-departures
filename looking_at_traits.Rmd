...
spp_master_spp <- data.frame(spp = unique(spp_master_ztemp_seus_buoy$spp), origin = "spp_master")
...
spp_master_ztemp_seus_buoy.traits <- spp_master_ztemp_seus_buoy[traits.u, nomatch = 0, on = c("spp")]
```

Now to save updated data.table with traits for species we have traits for
```{r save}
save(spp_master_ztemp_seus_buoy.traits, file = "spp_master_ztemp_seus_buoy.traits.Rdata")
```
...
spp_master_ztemp_seus_buoy.traits.na.age.max <- na.omit(spp_master_ztemp_seus_buoy.traits, cols="age.max")
spp_master_ztemp_seus_buoy.traits.na.age.max[,year_factor := as.factor(year)]
```

Does it hold up with this limited data set?
Now, I need to run all models again with this restricted data set
```{r run colonizatino models again with restricted data set to only age.max species}
#going to make loop to make all models I need to look at with single SURFACE variables
surface_variables <- colnames(spp_master_ztemp_seus_buoy.traits.na.age.max[,45:168])
...
for (i in 1:length(surface_variables)){
  mod <- glmer(col ~ get(surface_variables[i]) + (1|reg) + (1|year_factor), family = binomial, data = spp_master_ztemp_seus_buoy.traits.na.age.max)
...
bottom_variables <- colnames(spp_master_ztemp_seus_buoy.traits.na.age.max[,169:292]) #same as above
...
for (i in 1:length(bottom_variables)){
  mod <- glmer(col ~ get(bottom_variables[i]) + (1|reg) + (1|year_factor), family = binomial, data = spp_master_ztemp_seus_buoy.traits.na.age.max)
...
```
This is telling me that previous best model (max sbt temp change abs) is still good, but not the best... Therefore, it seems like we shoould be ignoring traits for now. Future analyses...

```{r}


mod.c.t.0 <- glmer(col ~ max_sbt_temp_change_abs + (1|reg) + (1|year_factor), family = binomial, data = spp_master_ztemp_seus_buoy.traits.na)
summary(mod.c.t.0)
#max_sbt_temp_change_abs	      0.16256 	0.00915	4242.65

#max_sbt_temp_change_abs and age.max
mod.c.t.1 <- glmer(col ~ max_sbt_temp_change_abs + age.max + (1|reg) + (1|year_factor), family = binomial, data = spp_master_ztemp_seus_buoy.traits.na)
summary(mod.c.t.1)
...
#interaction?
mod.c.t.2 <- glmer(col ~ max_sbt_temp_change_abs * age.max + (1|reg) + (1|year_factor), family = binomial, data = spp_master_ztemp_seus_buoy.traits.na)
summary(mod.c.t.2)
...
mod.c.t.3 <- glmer(col ~ age.max + (1|reg) + (1|year_factor), family = binomial, data = spp_master_ztemp_seus_buoy.traits.na)
...
#what's the best model now? #START HERE, run whole thing with restricted data set
#does it hold up with this limited data set?
mod.e.t.0 <- glmer(now_ext ~ seas_sbt_temp_change_abs_lag9 + (1|reg) + (1|year_factor), family = binomial, data = spp_master_ztemp_seus_buoy.traits.na)
summary(mod.e.t.0)
AICc(mod.e.t.0)

#seas_sbt_temp_change_abs_lag9	      0.08436 	0.222 	3937.797 * not significant any more

mod.e.t.0.1 <- glmer(now_ext ~ min_sst_temp_change_lag2 + (1|reg) + (1|year_factor), family = binomial, data = spp_master_ztemp_seus_buoy.traits.na)
summary(mod.e.t.0.1)
AICc(mod.e.t.0.1)

#min_sst_temp_change_lag2	             -0.14436 	0.00304 	3930.58

mod.e.t.0.2 <- glmer(now_ext ~ min_sbt_temp_change_lag7 + (1|reg) + (1|year_factor), family = binomial, data = spp_master_ztemp_seus_buoy.traits.na)
summary(mod.e.t.0.2)
AICc(mod.e.t.0.2)
...
mod.e.t.1 <- glmer(now_ext ~ seas_sbt_temp_change_abs_lag9 + age.max + (1|reg) + (1|year_factor), family = binomial, data = spp_master_ztemp_seus_buoy.traits.na)
summary(mod.e.t.1)
...
mod.e.t.1.1 <- glmer(now_ext ~ min_sst_temp_change_lag2 + age.max + (1|reg) + (1|year_factor), family = binomial, data = spp_master_ztemp_seus_buoy.traits.na)
summary(mod.e.t.1.1)
AICc(mod.e.t.0.1, mod.e.t.1.1)
  ...
  #min_sst_temp_change_lag2 and             -0.144548 0.048814  3889.425
      #                       age.max      -0.029182 4.7e-09
          #interaction?
                mod.e.t.1.1.i <- glmer(now_ext ~ min_sst_temp_change_lag2 * age.max + (1|reg) + (1|year_factor), family = binomial, data = spp_master_ztemp_seus_buoy.traits.na)
                summary(mod.e.t.1.1.i)
                AICc(mod.e.t.0.1, mod.e.t.1.1.i) #3890.142

#min_sbt_temp_change_lag7 and age.max
mod.e.t.1.2 <- glmer(now_ext ~ min_sbt_temp_change_lag7 + age.max + (1|reg) + (1|year_factor), family = binomial, data = spp_master_ztemp_seus_buoy.traits.na)
summary(mod.e.t.1.2)
...
#model with only age max
mod.e.t.1.r <- glmer(now_ext ~ age.max + (1|reg) + (1|year_factor), family = binomial, data = spp_master_ztemp_seus_buoy.traits.na)
summary(mod.e.t.1.r)
AICc(mod.e.t.1.r)
...
In all cases, adding age.max decreased the AIC value from not using. The negative, is that we lose 6000 observations. `spp_master_ztemp_seus_buoy` = 18932, `spp_master_ztemp_seus_buoy_traits` = 13296 and `spp_master_ztemp_seus_buoy_traits.na` (has age.max) = 12594

NEXT STEPS
Can we make a plot for how likelihood of colonization and extinction changes with age.max?
```{r age.max region bias?}
ggplot(aes(x=age.max), data = spp_master_ztemp_seus_buoy.traits.na) +
  geom_histogram() +
  facet_wrap(~reg)
...
```{r colonization with age of maturity}
hist(spp_master_ztemp_seus_buoy.traits$age.maturity)


#First, how does this model do with only rows that we actually have trait data for?
spp_master_ztemp_seus_buoy.traits.na.age.maturity <- na.omit(spp_master_ztemp_seus_buoy.traits, cols="age.maturity")
spp_master_ztemp_seus_buoy.traits.na.age.maturity[,year_factor := as.factor(year)]

#does it hold up with this limited data set?
mod.c.age.maturity.0 <- glmer(col ~ max_sbt_temp_change_abs + (1|reg) + (1|year_factor), family = binomial, data = spp_master_ztemp_seus_buoy.traits.na.age.maturity)
summary(mod.c.age.maturity.0)
AICc(mod.c.age.maturity.0)

#max_sbt_temp_change_abs  0.14627  0.0233 ** 	4069.372

#max_sbt_temp_change_abs and age.maturity
mod.c.age.maturity.1 <- glmer(col ~ max_sbt_temp_change_abs + age.maturity + (1|reg) + (1|year_factor), family = binomial, data = spp_master_ztemp_seus_buoy.traits.na.age.maturity)
summary(mod.c.age.maturity.1)
AICc(mod.c.age.maturity.0, mod.c.age.maturity.1)
...
#interaction?
mod.c.age.maturity.2 <- glmer(col ~ max_sbt_temp_change_abs * age.maturity + (1|reg) + (1|year_factor), family = binomial, data = spp_master_ztemp_seus_buoy.traits.na.age.maturity)
summary(mod.c.tl.2)
  #yes, appears that trophic level impacts interaction with temperature (p=0.003700), and it's the best model, AICc = 4509.615

AICc(mod.c.age.maturity.0, mod.c.age.maturity.1, mod.c.age.maturity.2)

#what about only age maturity?
mod.c.age.maturity.3 <- glmer(col ~ age.maturity + (1|reg) + (1|year_factor), family = binomial, data = spp_master_ztemp_seus_buoy.traits.na.age.maturity)
AICc(mod.c.age.maturity.3) 
...
hist(spp_master_ztemp_seus_buoy.traits$tl)


#First, how does this model do with only rows that we actually have trait data for?
spp_master_ztemp_seus_buoy.traits.na.tl <- na.omit(spp_master_ztemp_seus_buoy.traits, cols="tl")
spp_master_ztemp_seus_buoy.traits.na.tl[,year_factor := as.factor(year)]

#does it hold up with this limited data set?
mod.c.tl.0 <- glmer(col ~ max_sbt_temp_change_abs + (1|reg) + (1|year_factor), family = binomial, data = spp_master_ztemp_seus_buoy.traits.na.tl)
summary(mod.c.tl.0)
AICc(mod.c.tl.0)
...
#max_sbt_temp_change_abs and age.max
mod.c.tl.1 <- glmer(col ~ max_sbt_temp_change_abs + tl + (1|reg) + (1|year_factor), family = binomial, data = spp_master_ztemp_seus_buoy.traits.na.tl)
summary(mod.c.tl.1)
...
#interaction?
mod.c.tl.2 <- glmer(col ~ max_sbt_temp_change_abs * tl + (1|reg) + (1|year_factor), family = binomial, data = spp_master_ztemp_seus_buoy.traits.na.tl)
summary(mod.c.tl.2)
  #yes, appears that trophic level impacts interaction with temperature (p=0.003700), and it's the best model, AICc = 4509.615
...
mod.c.tl.3 <- glmer(col ~ tl + (1|reg) + (1|year_factor), family = binomial, data = spp_master_ztemp_seus_buoy.traits.na.tl)
AICc(mod.c.tl.3) # 4522.709
  #better with temperature as well

#one more... what about both tl and max age? are they correlated? they should be

lm <- lm(spp_master_ztemp_seus_buoy.traits$tl ~ spp_master_ztemp_seus_buoy.traits$age.max)
summary(lm)
plot(spp_master_ztemp_seus_buoy.traits$tl, spp_master_ztemp_seus_buoy.traits$age.max)
```
