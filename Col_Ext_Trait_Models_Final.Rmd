...
load("spp_master_ztemp_seus_buoy.traits.RData")
```
...
spp_master_ztemp_varsonly <- spp_master_ztemp_seus_buoy.traits[,c(1,45:292, 301, 308, 310, 312, 313)]

#names of temp variables, so I can add "_scaled" suffix
temptraitvars <- colnames(spp_master_ztemp_seus_buoy.traits[,c(45:292, 301, 308, 310, 312, 313)])
...
spp_master_ztemp_seus_buoy.traits_scaled <- cbind(spp_master_ztemp_seus_buoy.traits, spp_master_ztemp_varsonly[,scaled_cols, with = FALSE])

save(spp_master_ztemp_seus_buoy.traits_scaled, file = "spp_master_ztemp_seus_buoy.traits_scaled.RData")
...
cor(spp_master_ztemp_seus_buoy.traits_scaled[,.(age.max_scaled, age.maturity_scaled, tl_scaled, length.max_scaled, length.infinity_scaled)], use = "complete.obs")

pairs(spp_master_ztemp_seus_buoy.traits_scaled[,.(age.max_scaled, age.maturity_scaled, tl_scaled, length.max_scaled, length.infinity_scaled)])
...
tempvars <- names(spp_master_ztemp_seus_buoy.traits_scaled[,314:561])
...
#Colonization table
colonization.reduced <- spp_master_ztemp_seus_buoy.traits_scaled[,colstosaveforcol, with = F] #reduces to columns we need
...
#Colonization table
extinction.reduced <- spp_master_ztemp_seus_buoy.traits_scaled[,colstosaveforext, with = F] #reduces to columns we need
...
load("colonization_model_comparison_traits.RData")
...
load("extinction_model_comparison_traits.RData")
...
mod_col_trait_allspp <- glmer(col ~ seas_sst_temp_scaled * age.maturity_scaled + age.max_scaled + (1|reg) + (1|spp), family = "binomial", data = colonization.reduced, nAGQ = 0)


#temp only col trait model seas_sst_temp_scaled
mod_col_trait_notraits <- glmer(col ~ seas_sst_temp_scaled + (1|reg) + (1|spp), family = "binomial", data = colonization.reduced, nAGQ = 0)
summary(mod_col_trait_notraits)

#trait only col trait model
mod_col_trait_notemp <- glmer(col ~ age.maturity_scaled + age.max_scaled + (1|reg) + (1|spp), family = "binomial", data = colonization.reduced, nAGQ = 0)
summary(mod_col_trait_notemp)

#reverse to agematurity unscaled
mod_col_reverse_scaled_agematurity <- lm(age.maturity ~ age.maturity_scaled, data = spp_master_ztemp_seus_buoy.traits_scaled) 

mod_col_forward_scaled_agematurity <- lm(age.maturity_scaled ~ age.maturity, data = spp_master_ztemp_seus_buoy.traits_scaled)
...
ggplot(data = preds_predictors, aes(x = max_sst_temp_change_abs_lag1, y = preds, color = as.factor(ages_we_want))) +
  geom_line(size = 1.5) +
  labs(colour="Age at Maturity", x = "Absolute change in maximum surface temperature\nexperienced a year previously (˚C)", y = "Probability of Colonization",title = "Predicted Probabilities of Colonization") +
...
ggplot(data = preds_predictors, aes(x = max_sst_temp, y = preds, color = as.factor(ages_we_want))) +
  geom_line(size = 1.5) +
  labs(colour="Age at Maturity", x = "Maximum surface temperature (˚C)", y = "Probability of Extirpation",title = "Predicted Probabilities of Extirpation") +
  scale_y_continuous(breaks=c(0, 0.5, 1)) +
  theme_classic() +
  scale_color_manual(values = pal) +
  theme(text=element_text(size = 14))
...
ggplot(data = preds_predictors_seas, aes(x = seas_sst_temp_lag3, y = preds_seas, color = as.factor(ages_we_want))) +
  geom_line(size = 1.5) +
  labs(colour="Age at Maturity", x = "Temperature Seasonality\nThree Years Previously (˚C)", y = "Probability of Extirpation",title = "Predicted Probabilities of Extirpation") +
 # scale_y_continuous(breaks=c(0, 0.5, 1)) +
  theme_classic() +
  scale_color_manual(values = pal) +
  theme(text=element_text(size = 14))
...
bestcolmod_scaled_traits <- glmer(col ~ max_sst_temp_change_abs_lag1_scaled * age.maturity_scaled + age.max_scaled + (1|reg) + (1|spp), family = binomial, data = spp_master_ztemp_seus_buoy.traits_scaled, nAGQ = 0)

interceptcolmod_scaled_traits <- glmer(col ~ (1|reg) + (1|spp), family = binomial, data = spp_master_ztemp_seus_buoy.traits_scaled, nAGQ = 0)
...
model_imputs_col_traits <- spp_master_ztemp_seus_buoy.traits_scaled[,.(spp, reg, year, col, max_sst_temp_change_abs_lag1_scaled, age.maturity_scaled, age.max_scaled)]
...
bestextmod_scaled_traits <- glmer(now_ext ~ max_sst_temp_scaled * age.maturity_scaled + age.max_scaled + (1|reg) + (1|spp), family = binomial, data = spp_master_ztemp_seus_buoy.traits_scaled, nAGQ = 0)

interceptextmod_scaled_traits <- glmer(now_ext ~ (1|reg) + (1|spp), family = binomial, data = spp_master_ztemp_seus_buoy.traits_scaled, nAGQ = 0)
...
model_imputs_ext_traits <- spp_master_ztemp_seus_buoy.traits_scaled[,.(spp, reg, year, now_ext, max_sst_temp_scaled, age.maturity_scaled, age.max_scaled)]
