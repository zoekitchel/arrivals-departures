...
age_match <- data.table(ages_we_want = c(1, 10, 50, 80, 100))
...
temp_agemax_newdata <- mean(spp_master_ztemp_seus_buoy.traits_scaled$max_sst_temp_change_abs_lag1_scaled)
...
preds_predictors$ages_we_want <- factor(preds_predictors$ages_we_want, levels=c("100", "80", "50", "10", "1"))
...
hist(spp_master_ztemp_seus_buoy.traits$max_sst_temp, 100)

#second best ext trait model
mod_ext_trait_2 <- glmer(now_ext ~ max_sst_temp_lag1_scaled * age.maturity_scaled + age.max_scaled + (1|reg) + (1|spp), family = "binomial", data = extinction.reduced, nAGQ = 0)

#temp only ext trait model min_sst_temp_lag3_scaled
mod_ext_trait_notraits <- glmer(now_ext ~ min_sst_temp_lag3_scaled + (1|reg) + (1|spp), family = "binomial", data = extinction.reduced, nAGQ = 0)


#trait only ext trait model
mod_ext_trait_notemp <- glmer(now_ext ~ age.maturity_scaled + age.max_scaled + (1|reg) + (1|spp), family = "binomial", data = extinction.reduced, nAGQ = 0)
summary(mod_ext_trait_notemp)
...
temp_ext_newdata <- runif(500,min(spp_master_ztemp_seus_buoy.traits_scaled$max_sst_temp_scaled), max(spp_master_ztemp_seus_buoy.traits_scaled$max_sst_temp_change_scaled))

temp_agematurity_newdata <- age_match[,age.maturity_scaled]


temp_agemax_newdata <- mean(spp_master_ztemp_seus_buoy.traits_scaled$max_sst_temp_scaled)
...
preds_ext <- predict(mod_ext_trait, newdata = ext_newdata_traits, re.form = NA, type = "response")
preds_predictors_ext <- data.table(cbind(ext_newdata_traits, preds_ext))
preds_predictors_ext <- preds_predictors_ext[age_match, on = "age.maturity_scaled"]

#I want x axis to be meaningful
mod_ext_reverse_scaledtemp <- lm(max_sst_temp ~ max_sst_temp_scaled, data = spp_master_ztemp_seus_buoy.traits_scaled)


preds_predictors_ext[,max_sst_temp := mod_ext_reverse_scaledtemp$coefficients[2]*max_sst_temp_scaled + mod_ext_reverse_scaledtemp$coefficients[1] ]

#reverse order of ages to match
preds_predictors_ext$ages_we_want <- factor(preds_predictors_ext$ages_we_want, levels=c("100", "80", "50", "10", "1"))


ggplot(data = preds_predictors_ext, aes(x = max_sst_temp, y = preds, color = as.factor(ages_we_want))) +
  geom_line(size = 1.5) +
