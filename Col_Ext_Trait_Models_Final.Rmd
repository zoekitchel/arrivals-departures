...
#best colonization trait models (all delta less than 2)
  #max_sst_temp_change_abs_lag1_scaled * age.maturity_scaled + age.max_scaled 0.00
  #seas_sst_temp_change_abs_scaled + age.maturity_scaled + age.max_scaled 1.43
...
model_imputs <- spp_master_ztemp.traits_scaled[,.(spp, reg, year, col, now_ext, max_sst_temp_change_abs_lag1_scaled, age.maturity_scaled, age.max_scaled)]

model_imputs <- na.omit(model_imputs)

t2_col <- predict(bestcolmod_scaled_traits, newdata = model_imputs, type = "response", re.form = NULL) #setting re.form to NULL includes all random effects
...
#best extinction trait models (all delta less than 2)
 ...
 #max_sst_temp_lag1_scaled    * age.maturity_scaled + age.max_scaled 1.884106
...
model_imputs <- spp_master_ztemp.traits_scaled[,.(spp, reg, year, col, now_ext, max_sst_temp_scaled, age.maturity_scaled, age.max_scaled)]

model_imputs <- na.omit(model_imputs)

t2_ext <- predict(bestextmod_scaled_traits, newdata = model_imputs, type = "response", re.form = NULL) #setting re.form to NULL includes all random effects
