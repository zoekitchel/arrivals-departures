...
col_model_comparisons_traits <- model_imputs_col_traits[, .(col_predicted = sum(pred), col_observed = sum(col)), by=.(year,reg)]
...
ext_model_comparisons_traits <- model_imputs_ext_traits[, .(ext_predicted = sum(pred), ext_observed = sum(now_ext)), by=.(year,reg)]
...
#from colonizations
cumulative_richness_traits <- col_model_comparisons_traits[,.(reg, year, col_obs_sum, col_pred_sum)]
...
cumulative_richness_traits[, Observed := col_obs_sum-ext_obs_sum][, Predicted := col_pred_sum-ext_pred_sum]

cumulative_richness_traits.r <- cumulative_richness_traits[,.(reg, year, Observed, Predicted)]

#wide to long
cumulative_richness_traits.r <- melt(cumulative_richness_traits.r, id.vars = c("reg", "year"), measure.vars = c("Observed", "Predicted"), variable.name = "Richness", value.name = "value")
...
cum_richness_plot_traits <- ggplot(aes(x=year, y = value, color = Richness), data = cumulative_richness_traits.r) +
  geom_point(size = 2) +
