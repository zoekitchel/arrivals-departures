...
mod_col_null <- glmer(col ~ (1|reg) + (1|year), family = binomial, data = spp_master_ztemp)
AICc(mod_col_null)

#delta AIC between null and best model
AICc(mod_col_null)-AICc(mod_col)
...
#null model (intercept only)
mod_ext_null <- glmer(now_ext ~ (1|reg) + (1|year), family = binomial, data = spp_master_ztemp)

#delta AIC between null and best model
AICc(mod_ext_null)-AICc(mod_ext)
