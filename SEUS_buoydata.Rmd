...
ggplot(data = data_over_time.u, aes(x = YYYY, y = Location, fill = as.factor(exist))) +
  ...
  theme_classic() +
  labs(x="Year", "Buoy ID and Location", fill = "Data Availability")
...
load("/Users/zoekitchel/Documents/grad school/Rutgers/Repositories/Col_Ext/seus_temp_mod.RData")
...
for (i in first_year:nrow(predicted_sst_seus_monthly_avg)) {
  ...
  #surface raw
    ...
    predicted_sst_seus_monthly_avg[(i),mean_sst_temp :=mean(subset_sst$sst_month_sst)] #mean temp over the past 12 months
...
load("seus_temp_final_frombuoys.RData")

#plot avg temp over time

ggplot(data = seus_temp_final, aes(x = year)) +
  geom_line(aes(y = mean_sst_temp), color = "black") +
  geom_line(aes(y = max_sst_temp), color = "red") +
  geom_line(aes(y = min_sst_temp), color = "blue") +
...
(buoypoints <- ggplot(data = southeast) +
	...
	xlab("Longitude (˚E)") +
	ylab("Latitude (˚N)") +
  scalebar(southeast, dist = 100, dist_unit = "km",
             transform = TRUE, model = "WGS84", border.size = 0.6, height = 0.013, st.size = 2))
